...
{
    public static Judge2 judge2;

    [SerializeField]
    public List<(GameObject note, int legnth, int ms)> Note2;

    public int nowOnIndex2;
    ...
    {
        judge2 = this;
        Note2 = new List<(GameObject note, int legnth, int ms)>();
    }
    ...
    private void Update()
    {
        ...
        try
        {
            judge_ms = Note2[nowOnIndex2].ms - MainJudgeManage.mainJudge.GameMS;
            ...
            else if (Note2[nowOnIndex2].legnth != 0)
            {
                int num;
                ...
                if (longNoteJudgeMs >= Note2[nowOnIndex2].legnth)
                {
                    longNoteJudgeMs = 0;
                    Note2[nowOnIndex2].note.SetActive(false);
                    nowOnIndex2++;
                }
            }
            else if (judge_ms < -85)
